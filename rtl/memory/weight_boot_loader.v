// weight_boot_loader.v - Parses a packed weight image and copies payloads to DDR3.
// (Phase 8 note: `default_nettype none` removed because port list uses implicit
//  wire declarations; switching it on cleanly requires retyping every port with
//  an explicit `wire`/`reg` keyword. Tracked as future RTL hygiene work in
//  docs/asic_readiness_report.md §6.)
//
// Input stream is the binary format generated by tools/pack_weight_image.py:
// 64-byte header, N*64-byte table, then aligned payload blobs. Header/table are
// validated, payloads are byte-copied to DDR3, and CRC32 of each payload is
// computed in hardware and compared against the manifest CRC. A mismatch
// raises `crc_error` and aborts the load via the ST_ERROR path.
module weight_boot_loader #(
    parameter MAX_ENTRIES   = 16,
    parameter CMD_WRITE     = 3'b000,
    parameter ENFORCE_CRC32 = 1            // 1=enforce, 0=parse-only (legacy)
)(
    input              sys_clk,
    input              rst_n,
    ...
    output reg         done,
    output reg         error,
    output reg         crc_error,           // last payload CRC mismatch flag
...
localparam [63:0] MAGIC = 64'h3154_4757_4445_4d42; // "BMEDWGT1" little endian
...
// ---- CRC32 streaming over payload bytes ----
// Cleared at the start of each ST_START_WR (per-payload), updated for every
// payload byte consumed in ST_COPY. Sampled in ST_WAIT_WR after writer_done.
reg         crc_clear_r;
    ...
    if (!rst_n) begin
        ...
        case (state)
            ...
            ST_START_WR: begin
                payload_count <= 32'd0;
                writer_start  <= 1'b1;
                crc_clear_r   <= 1'b1;    // reset CRC accumulator for new payload
                state         <= ST_COPY;
            end
            ...
            ST_WAIT_WR: begin
                if (writer_done) begin
                    // ---- CRC enforce per payload (optional, parameter-gated) ----
                    if (ENFORCE_CRC32 == 1 &&
